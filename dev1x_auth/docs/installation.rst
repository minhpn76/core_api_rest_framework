...
2. Add ``mind_auth`` app to INSTALLED_APPS in your django settings.py:
...
.. note:: This project depends on ``django-rest-framework`` library, so install it if you haven't done yet. Make sure also you have installed ``mind_core`` and ``mind_core.authtoken`` apps

3. Add mind_auth urls:

.. code-block:: python

    urlpatterns = [
        ...,
        url(r'^rest-auth/', include('mind_auth.urls'))
    ]
...
2. Add ``django.contrib.sites``, ``allauth``, ``allauth.account`` and ``mind_auth.registration`` apps to INSTALLED_APPS in your django settings.py:
...
3. Add mind_auth.registration urls:
...
.. code-block:: python
    ...
    urlpatterns = [
        ...,
        url(r'^rest-auth/', include('mind_auth.urls')),
        url(r'^rest-auth/registration/', include('mind_auth.registration.urls'))
    ]
...
3. Create new view as a subclass of ``mind_auth.registration.views.SocialLoginView`` with ``FacebookOAuth2Adapter`` adapter as an attribute:

.. code-block:: python

    from allauth.socialaccount.providers.facebook.views import FacebookOAuth2Adapter
    from mind_auth.registration.views import SocialLoginView
...
3. Create new view as a subclass of ``mind_auth.views.LoginView`` with ``TwitterOAuthAdapter`` adapter and  ``TwitterLoginSerializer`` as an attribute:

.. code-block:: python

    from allauth.socialaccount.providers.twitter.views import TwitterOAuthAdapter
    from mind_auth.registration.views import SocialLoginView
    from mind_auth.social_serializers import TwitterLoginSerializer
...
.. code-block:: python
    ...
    from allauth.socialaccount.providers.facebook.views import FacebookOAuth2Adapter
    from mind_auth.registration.views import SocialConnectView
    from mind_auth.social_serializers import TwitterConnectSerializer
...
.. code-block:: python
    ...
    from mind_auth.registration.views import (
