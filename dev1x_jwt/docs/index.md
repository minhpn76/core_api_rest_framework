...
```python
    from mind_jwt.views import refresh_jwt_token
    #  ...
...
```python
    from mind_jwt.views import verify_jwt_token
...
### JWT_ALLOW_REFRESH
Enable token refresh functionality. Token issued from `mind_jwt.views.obtain_jwt_token` will have an `orig_iat` field. Default is `False`
