...
from mind_core.compat import apply_markdown, pygments_highlight
from mind_core.renderers import HTMLFormRenderer
from mind_core.utils.urls import replace_query_param
...
def optional_login(request):
    ...
    """
    try:
        login_url = reverse('mind_core:login')
...
def optional_logout(request, user):
    ...
    """
    try:
        logout_url = reverse('mind_core:logout')
    except NoReverseMatch:
...
def format_value(value):
    ...
    if value is None or isinstance(value, bool):
        ...
        if any([isinstance(item, (list, dict)) for item in value]):
            template = loader.get_template('mind_core/admin/list_value.html')
        else:
            template = loader.get_template('mind_core/admin/simple_list_value.html')
        context = {'value': value}
        return template.render(context)
    elif isinstance(value, dict):
        template = loader.get_template('mind_core/admin/dict_value.html')
        context = {'value': value}
        return template.render(context)
