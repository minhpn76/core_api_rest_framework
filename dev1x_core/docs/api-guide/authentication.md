...
The default authentication schemes may be set globally, using the `DEFAULT_AUTHENTICATION_CLASSES` setting.  For example.
    ...
    mind_core = {
        'DEFAULT_AUTHENTICATION_CLASSES': (
            'mind_core.authentication.BasicAuthentication',
            'mind_core.authentication.SessionAuthentication',
        )
    }
...
using the `APIView` class-based views.
    ...
    from mind_core.authentication import SessionAuthentication, BasicAuthentication
    from mind_core.permissions import IsAuthenticated
...
To use the `TokenAuthentication` scheme you'll need to [configure the authentication classes](#setting-the-authentication-scheme) to include `TokenAuthentication`, and additionally include `mind_core.authtoken` in your `INSTALLED_APPS` setting:
...
**Note:** Make sure to run `manage.py migrate` after changing your settings. The `mind_core.authtoken` app provides Django database migrations.
...
* `request.user` will be a Django `User` instance.
* `request.auth` will be a `mind_core.authtoken.models.Token` instance.
...
For example, you may return additional user information beyond the `token` value:
    ...
    from mind_core.authtoken.views import ObtainAuthToken
    from mind_core.authtoken.models import Token
    from mind_core.response import Response
...
`your_app/admin.py`:
    ...
    from mind_core.authtoken.admin import TokenAdmin
...
The following example will authenticate any incoming request as the user given by the username in a custom request header named 'X_USERNAME'.
	...
	from django.contrib.auth.models import User
    from mind_core import authentication
    from mind_core import exceptions
...
Add the package to your `INSTALLED_APPS` and modify your REST framework settings.
    ...
    mind_core = {
        'DEFAULT_AUTHENTICATION_CLASSES': (
            'oauth2_provider.contrib.mind_core.OAuth2Authentication',
        )
    }
