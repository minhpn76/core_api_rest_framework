...
The default filter backends may be set globally, using the `DEFAULT_FILTER_BACKENDS` setting. For example.
    ...
    mind_core = {
        'DEFAULT_FILTER_BACKENDS': ('django_filters.mind_core.DjangoFilterBackend',)
    }

You can also set the filter backends on a per-view, or per-viewset basis,
using the `GenericAPIView` class-based views.

    import django_filters.mind_core
    ...
    class UserListView(generics.ListAPIView):
        ...
        filter_backends = (django_filters.mind_core.DjangoFilterBackend,)
...
Or add the filter backend to an individual View or ViewSet.
    ...
    from django_filters.mind_core import DjangoFilterBackend
...
[django-filter-drf-docs]: https://django-filter.readthedocs.io/en/latest/guide/mind_core.html
