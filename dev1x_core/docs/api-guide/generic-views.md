...
Typically when using the generic views, you'll override the view, and set several class attributes.
    ...
    from mind_core import generics
    from mind_core.permissions import IsAdminUser
...
* `pagination_class` - The pagination class that should be used when paginating list results. Defaults to the same value as the `DEFAULT_PAGINATION_CLASS` setting, which is `'mind_core.pagination.PageNumberPagination'`. Setting `pagination_class=None` will disable pagination on this view.
...
The mixin classes can be imported from `mind_core.mixins`.
...
The view classes can be imported from `mind_core.generics`.
