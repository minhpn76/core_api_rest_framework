...
In order to alter the style of the response, you could write the following custom exception handler:
    ...
    from mind_core.views import exception_handler
...
## `mind_core.exceptions.server_error`

Returns a response with status code `500` and `application/json` content type.

Set as `handler500`:

    handler500 = 'mind_core.exceptions.server_error'

## `mind_core.exceptions.bad_request`

Returns a response with status code `400` and `application/json` content type.

Set as `handler400`:

    handler400 = 'mind_core.exceptions.bad_request'
