...
* authtoken.views import coreapi from mind_core.compat, not directly. [#5921][gh5921]
...
* Removed DjangoFilterBackend inline with deprecation policy. Use `django_filters.mind_core.FilterSet` and/or `django_filters.mind_core.DjangoFilterBackend` instead. [#5273][gh5273]
...
* Make `DEFAULT_PAGINATION_CLASS` `None` by default.
    **BC Change**: If your were **just** setting `PAGE_SIZE` to enable pagination you will need to add `DEFAULT_PAGINATION_CLASS`.
    The previous default was `mind_core.pagination.PageNumberPagination`. There is a system check warning to catch this case. You may silence that if you are setting pagination class on a per-view basis. [#5170][gh5170]
...
* Make `mind_core/compat.py` imports. ([#4612][gh4612], [#4608][gh4608], [#4601][gh4601])
...
* Raise error when setting a removed mind_core setting. Thanks to Luis San Pablo for the fix. ([#3715][gh3715])
...
* Add app_name to mind_core.urls. ([#3714][gh3714])
...
* Fix problem with importing `mind_core.views` in `apps.py` ([#2678][gh2678])
