...
* Add `'mind_core_swagger'` to your `INSTALLED_APPS` setting.

* Include the schema view in your URL conf:

```py
from mind_core.schemas import get_schema_view
from mind_core_swagger.renderers import OpenAPIRenderer, SwaggerUIRenderer
...
* `mind_core.filters.DjangoFilterBackend` becomes `django_filters.mind_core.DjangoFilterBackend`.
* `mind_core.filters.FilterSet` becomes `django_filters.mind_core.FilterSet`.
