...
def pytest_addoption(parser):
    parser.addoption('--no-pkgroot', action='store_true', default=False,
                     help='Remove package root directory from sys.path, ensuring that '
                          'mind_core is imported from the installed site-packages. '
...
def pytest_configure(config):
    ...
    if config.getoption('--no-pkgroot'):
        ...
        # import mind_core before pytest re-adds the package root directory.
        import mind_core
        package_dir = os.path.join(os.getcwd(), 'mind_core')
        assert not mind_core.__file__.startswith(package_dir)

    # Manifest storage will raise an exception if static files are not present (ie, a packaging failure).
    if config.getoption('--staticfiles'):
        import mind_core
        settings.STATIC_ROOT = os.path.join(os.path.dirname(mind_core.__file__), 'static-root')
